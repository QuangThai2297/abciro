 ...
 ***********************************************************************************************************************/
/*******************************************************************************************************************//**
 * @defgroup R_TOUCH Middleware Function Definitions
...
#include "r_touch_rx_config.h"
#include "r_touch_rx_if.h"
#include "r_touch_rx_private.h"
...
static void touch_common_callback(void* p_cb_arg);
static uint8_t already_open(touch_ctrl_blk_t const*const p_ctrl);
static touch_err_t read_sensor_parameter(touch_ctrl_blk_t * const p_ctrl, touch_read_t const * const p_arg);
static touch_err_t write_sensor_parameter(touch_ctrl_blk_t * const p_ctrl, touch_control_arg_t const *const p_arg);
static uint16_t get_sensor_offset(touch_ctrl_blk_t * const p_ctrl, uint8_t rx_ch, uint8_t tx_ch);

static ctsu_err_t filter(touch_ctrl_blk_t * const p_ctrl);
static uint16_t sensors_touched(touch_ctrl_blk_t * const p_ctrl);
static void drift_compensation(touch_ctrl_blk_t * const p_ctrl);
static ctsu_err_t sensitivity_optimization(touch_ctrl_blk_t * const p_ctrl);
 ...
 */
static touch_ctrl_blk_t pvt_touch_ctrl_blk[TOUCH_CFG_MAX_CONTROL_BLOCK_COUNT];
...
{
    return ((((uint32_t)TOUCH_VERSION_MAJOR) << 16) | (uint32_t)TOUCH_VERSION_MINOR);
}
/***********************************************************************************************************************
...
touch_err_t R_TOUCH_Open(uint32_t * const p_id, touch_cfg_t const * const p_cfg)
{
    ...
    ASSERT(p_id!=NULL);
    ASSERT(p_id < (uint32_t *)BSP_RAM_SIZE_BYTES);
    ASSERT(p_cfg!=NULL);
    ASSERT(p_cfg->buffer.p_start != NULL);
    ASSERT(p_cfg->buffer.p_start < (uint8_t *)BSP_RAM_SIZE_BYTES);
    ASSERT(p_cfg->p_binary_result != NULL);
    ASSERT(p_cfg->p_binary_result < (uint8_t *)BSP_RAM_SIZE_BYTES);
    ASSERT(p_cfg->p_common != NULL);
    ASSERT(p_cfg->p_sensor != NULL);
#endif
    touch_ctrl_blk_t * p_ctrl;
    uint8_t * ch_en;
    ...
    touch_ctrl_blk_t ctrl_blk_local =
    {
        ...
        .num_touched = 0,
#if (MULTITOUCH_REJECTION_TYPE > 0)
        .multi_touch = false,
#endif
    ...
    ctsu_err_t ctsu_err = R_CTSU_Open((uint32_t*)&ctrl_blk_local.ctsu_hw_id, &ctsu.ctsu_cfg_copy);
    ...
    ctsu_err |= R_CTSU_Control(ctrl_blk_local.ctsu_hw_id, &ctsu.ctsu_control_arg);
#if (TOUCH_CFG_PARAM_CHECKING_ENABLE==true)
    ...
    if(CTSU_SUCCESS==ctsu_err)
    {
        uint32_t touch_hdl = already_open(&ctrl_blk_local);
        if(TOUCH_CFG_MAX_CONTROL_BLOCK_COUNT > touch_hdl)
        {
            /* Block already open */
            uint8_t callback_index = 0;
            p_ctrl = &pvt_touch_ctrl_blk[touch_hdl];
            for(uint8_t itr = 0 ; itr < TOUCH_CFG_MAX_UPPER_LAYER_CALLBACKS; itr++)
            {
                ...
                else if (p_ctrl->p_callback[itr] == p_cfg->p_callback)
                {
                    callback_index = itr;
                    break;
                }
            ...
            p_ctrl->p_callback[callback_index] = p_cfg->p_callback;
            p_ctrl->upper_level_count += 1;
            *p_id = touch_hdl;
        ...
        if(TOUCH_CFG_MAX_CONTROL_BLOCK_COUNT <= touch_hdl)
        {
            ...
            for(touch_hdl = 0; touch_hdl < TOUCH_CFG_MAX_CONTROL_BLOCK_COUNT; touch_hdl++)
            {
                if (OPEN != pvt_touch_ctrl_blk[touch_hdl].open)
                {
                    /* Found unused handle */
                    p_ctrl = &pvt_touch_ctrl_blk[touch_hdl];
                    * p_id = touch_hdl;
            ...
            memset(ctrl_blk_local.p_callback, 0, TOUCH_CFG_MAX_UPPER_LAYER_CALLBACKS* sizeof(fit_callback_t));
            ctrl_blk_local.p_callback[TOUCH_CFG_MAX_UPPER_LAYER_CALLBACKS-1] = p_cfg->p_callback;
#if (TOUCH_CFG_PARAM_CHECKING_ENABLE==true)
            ASSERT(touch_hdl < TOUCH_CFG_MAX_CONTROL_BLOCK_COUNT);
#endif
        }
    }
#if (TOUCH_CFG_PARAM_CHECKING_ENABLE==true)
    ...
    {
        R_CTSU_Close(ctrl_blk_local.ctsu_hw_id);
    ...
    for(uint16_t itr = 0; itr < ctrl_blk_local.num_sensors; itr++)
    {
        ...
        /* 2017-06-27: REL has requested to turn-off sensor optimization at run-time. */
        p_sensor_info[itr].operation.bit.sensor_optimize = 0;
        ...
        p_sensor_info[itr].p_filter_state = ((NULL!=p_cfg->custom.p_filter) ?
                (p_cfg->custom.p_filter_instance[itr]):(void*)FIT_NO_PTR);
    }
...
#endif
            offset = (ch_en[p_cfg->p_ignored[itr].rx]*ctrl_blk_local.num_tx)+ch_en[p_cfg->p_ignored[itr].tx];
        }
        else
        {
    ...
    memcpy(p_ctrl, &ctrl_blk_local, sizeof(touch_ctrl_blk_t));
    p_ctrl->upper_level_count = 1;
...
touch_err_t R_TOUCH_Close(uint32_t const id)
{
    ctsu_err_t ctsu_err;
#if (TOUCH_CFG_PARAM_CHECKING_ENABLE==true)
    ASSERT(TOUCH_CFG_MAX_CONTROL_BLOCK_COUNT > id);
#endif
    touch_ctrl_blk_t * p_ctrl = &pvt_touch_ctrl_blk[id];
#if (TOUCH_CFG_PARAM_CHECKING_ENABLE==true)
    ASSERT(OPEN == p_ctrl->open);
#endif
    /** a. Check resource lock */
    if( false==R_BSP_SoftwareLock(&p_ctrl->lock) )
    {
    ...
    /** b. Close lower level  */
    ctsu_err = R_CTSU_Close(p_ctrl->ctsu_hw_id);
    if( CTSU_SUCCESS != ctsu_err)
    {
...
#if (TOUCH_CFG_CHECK_UPPER_LEVEL_COUNT==true)
    if(0 == (--p_ctrl->upper_level_count))
#endif
    {
        /* No more upper levels using the control block. */
        memset(p_ctrl, 0, sizeof(touch_ctrl_blk_t));
    }
#if (TOUCH_CFG_CHECK_UPPER_LEVEL_COUNT==true)
    else
#endif
    {
        /* Unlock handle because we locked in step a. */
        R_BSP_SoftwareUnlock(&p_ctrl->lock);
    }
...
***********************************************************************************************************************/
touch_err_t R_TOUCH_StartScan(uint32_t id)
{
    ctsu_err_t ctsu_err;
#if (TOUCH_CFG_PARAM_CHECKING_ENABLE==true)
    ASSERT(TOUCH_CFG_MAX_CONTROL_BLOCK_COUNT > id);
    ...
    ctsu_err = R_CTSU_Scan(p_ctrl->ctsu_hw_id);
...
* Return Value : TOUCH_SUCCESS, TOUCH_ERR_LOCKED,
***********************************************************************************************************************/
touch_err_t R_TOUCH_Update(uint32_t id)
{
    touch_callback_arg_t cb_args;
    uint8_t touched_sensor_count = 0;
    ctsu_err_t ctsu_err;
#if (TOUCH_CFG_PARAM_CHECKING_ENABLE==true)
    ASSERT(TOUCH_CFG_MAX_CONTROL_BLOCK_COUNT > id);
#endif
    ...
    touched_sensor_count = sensors_touched(p_ctrl);

    /* Reset the drift hold count and on counter if number of sensors touched has changed*/
    if(touched_sensor_count != p_ctrl->num_touched)
    {
        p_ctrl->drift_hold_count = p_ctrl->drift_hold_limit;
        p_ctrl->on_count = 0;
    }
    ...
    if (0 == touched_sensor_count)
    {
        ...
        p_ctrl->drift_hold_count = 0;
        p_ctrl->on_count = 0;
    }

    p_ctrl->num_touched = touched_sensor_count;
    ...
    drift_compensation(p_ctrl);

    p_ctrl->drift_hold_count -= (((p_ctrl->drift_hold_count > 0)
            &&(touched_sensor_count > 0)) ?
                    (1U):(0));
    p_ctrl->on_count += (((p_ctrl->on_count <= p_ctrl->on_limit)
            &&(touched_sensor_count > 0)) ?
                    (1U):(0));

    sensitivity_optimization(p_ctrl);
...
touch_err_t R_TOUCH_Read(uint32_t id, touch_read_t const * const p_arg)
{
    ...
    ASSERT(TOUCH_CFG_MAX_CONTROL_BLOCK_COUNT > id);
#endif
    touch_ctrl_blk_t * const p_ctrl = &pvt_touch_ctrl_blk[id];
#if (TOUCH_CFG_PARAM_CHECKING_ENABLE==true)
    ASSERT(OPEN == p_ctrl->open);
#endif
    /** a. Check resource lock */
    ...
    if(TOUCH_DATA_BINARY==p_arg->read_cmd)
    {
...
#endif
            byte_offset = sensor_offset>>3;
            bit_offset  = sensor_offset & 7;
            bit_mask = (1<<bit_offset);
            result = p_ctrl->p_binary_result[byte_offset];
        ...
        for (itr = 0; (itr < p_ctrl->num_sensors) && (0 == p_arg->sensor_count); itr += 8)
        {
            byte_offset = itr>>3;
            result = p_ctrl->p_binary_result[byte_offset];
    ...
    else
    {
        switch(p_arg->read_cmd)
        {
            case TOUCH_DATA_CTSU_HW_IDENTIFIER:
                memcpy(p_arg->p_dest, &p_ctrl->ctsu_hw_id, sizeof(p_ctrl->ctsu_hw_id));
                break;
            case TOUCH_DATA_OPERATION_FLAGS:
                touch_err = read_sensor_parameter(p_ctrl, p_arg);
                break;
            case TOUCH_DATA_COMMON_DRIFT_HOLD_INTERVAL:
                memcpy(p_arg->p_dest, &p_ctrl->drift_hold_limit, sizeof(uint16_t));
                break;
            case TOUCH_DATA_COMMON_DRIFT_HOLD_COUNTER:
                memcpy(p_arg->p_dest, &p_ctrl->drift_hold_count, sizeof(uint16_t));
                break;
            case TOUCH_DATA_COMMON_CONTINUOUS_ON_INTERVAL:
                memcpy(p_arg->p_dest, &p_ctrl->on_limit, sizeof(uint16_t));
                break;
            case TOUCH_DATA_COMMON_CONTINUOUS_ON_COUNTER:
                memcpy(p_arg->p_dest, &p_ctrl->on_count, sizeof(uint16_t));
                break;
            case TOUCH_DATA_SENSOR_OFFSET:
                for(itr = 0; itr < p_arg->sensor_count; itr++)
                {
                    uint16_t * p_result = p_arg->p_dest;
                    p_result[itr] = get_sensor_offset(p_ctrl, p_arg->sensor[itr].rx, p_arg->sensor[itr].tx);
                }
                break;
            default:
                touch_err = read_sensor_parameter(p_ctrl, p_arg);
...
touch_err_t R_TOUCH_Control(uint32_t const id, touch_control_arg_t const * const p_arg)
{
    ...
    {
        memcpy((void*)&p_ctrl->ctsu_hw_id, p_arg->p_dest, sizeof(p_ctrl->ctsu_hw_id));
    }
    else if (TOUCH_DATA_OPERATION_FLAGS == p_arg->cmd)
    {
        touch_err = write_sensor_parameter(p_ctrl, p_arg);
    }
    ...
    else
    {
        ...
        {
            case TOUCH_DATA_COMMON_DRIFT_HOLD_INTERVAL:
                memcpy((void*)&p_ctrl->drift_hold_limit, p_arg->p_dest, sizeof(uint16_t));
                break;

            case TOUCH_DATA_COMMON_DRIFT_HOLD_COUNTER:
                memcpy(&p_ctrl->drift_hold_count, p_arg->p_dest, sizeof(uint16_t));
                break;

            case TOUCH_DATA_COMMON_CONTINUOUS_ON_INTERVAL:
                memcpy((void*)&p_ctrl->on_limit, p_arg->p_dest, sizeof(uint16_t));
                break;

            case TOUCH_DATA_COMMON_CONTINUOUS_ON_COUNTER:
                memcpy(&p_ctrl->on_count, p_arg->p_dest, sizeof(uint16_t));
                break;
...
touch_err_t R_TOUCH_Calibrate(uint32_t const id)
{
    ...
    uint16_t num_sensors_to_tune = (p_ctrl->num_sensors - p_ctrl->num_ignored);

    ctsu_err = R_CTSU_Calibrate(p_ctrl->ctsu_hw_id);
#if (TOUCH_CFG_PARAM_CHECKING_ENABLE==true)
    ...
    while(num_tuned_sensors < num_sensors_to_tune)
    {
        ...
        touch_err = R_TOUCH_StartScan(id);
#if (TOUCH_CFG_PARAM_CHECKING_ENABLE==true)
        ASSERT(CTSU_SUCCESS==touch_err);
#endif
        /* Wait for scan to complete */
        while (false == R_BSP_SoftwareLock (&p_ctrl->lock))
        {
            /* GSCE CODAN Elimination: Do Nothing */;
        }

        if(p_ctrl->scan_errors > 0)
        {
        ...
        ignore_itr = 0;
        p_ignore = (touch_sensor_t*)&p_ctrl->p_ignored[ignore_itr];
        if(p_ctrl->num_ignored > 0)
        {
            itr_ignored = get_sensor_offset(p_ctrl, p_ignore->rx, p_ignore->tx);
            ignore_itr+=1;
        }

        for(itr = 0; itr < p_ctrl->num_sensors; itr++)
        {
            /* Read sensor results*/
            if((p_ctrl->num_ignored > 0) && (itr == itr_ignored))
            {
            ...
            ctsu_err = R_CTSU_Read(p_ctrl->ctsu_hw_id, &ctsu_read_args);
#if (TOUCH_CFG_PARAM_CHECKING_ENABLE==true)
            ASSERT(CTSU_SUCCESS==ctsu_err);
#endif
            ...
            ASSERT(CTSU_SUCCESS==ctsu_err);
#endif
            int16_t abs_del = abs(ctsusc_primary - ctsurc_primary);
            if( abs_del > (TUNING_UPPER_LIMIT + TUNING_LOWER_LIMIT))
            {
                ...
                ctsu_err = R_CTSU_Control(p_ctrl->ctsu_hw_id, &ctsu_control_arg);
#if (TOUCH_CFG_PARAM_CHECKING_ENABLE==true)
                ASSERT(CTSU_SUCCESS==ctsu_err);
#endif
            ...
            {   /* Counter values within range*/
                num_tuned_sensors += 1;
                ctsu_read_args.p_dest = &p_ctrl->p_sensor[itr].input;
...
*                   @note: Will return value > TOUCH_CFG_MAX_CONTROL_BLOCK_COUNT if not found.
***********************************************************************************************************************/
static uint8_t already_open(touch_ctrl_blk_t const*const p_ctrl)
{
    uint8_t itr;

    for(itr = 0; itr < TOUCH_CFG_MAX_CONTROL_BLOCK_COUNT; itr++)
    {
        if(OPEN==pvt_touch_ctrl_blk[itr].open)
        {
            if(0==memcmp(&pvt_touch_ctrl_blk[itr], p_ctrl, offsetof(touch_ctrl_blk_t, open)))
            {
...
static bool is_touched(void * p_arg)
{
    ...
    {
        dt_count += ((dt_count < dt_limit ) ? 1U: 0);
    ...
    else if (abs_delta < _builtin_abs (threshold - hysteresis))
    {
        dr_count = (dr_count > 0) ? (dr_count - 1U) : 0;
        if (0 == dr_count)
        {
            dt_count = 0;
        }
    ...
    p_sensor->delta = abs_delta & UINT16_MAX;
...
static void sensor_optimize_sensitivity(void * p_arg)
{
    ...
    avg_sen_cnt_pri = (avg_sen_cnt_pri >> 1) + (sen_cnt_pri >> 1);
    avg_ref_cnt_pri = (avg_ref_cnt_pri >> 1) + (ref_cnt_pri >> 1);

    int32_t abs_delta = abs(sen_cnt_pri - ref_cnt_pri);

    count = ( abs_delta > threshold ) ? (count+1): 0;
    ...
    if(count > interval)
    {
        ...
        ctsu_err = R_CTSU_Control(p_optimize_args->ctsu_hw_id, &arg);
#if (TOUCH_CFG_PARAM_CHECKING_ENABLE==true)
        /*ASSERT(CTSU_SUCCESS == ctsu_err); */
#endif
        ...
        {
            ctsuso += ((ctsuso < 0x3FE) ? 1:0);
        }
        else if(avg_sen_cnt_pri < avg_ref_cnt_pri)
        {
            ctsuso -= ((ctsuso > 1U) ? 1:0);
        }
        else
        {
...
static void filter_2n_lpraf(void* p_arg)
{
     ...
     */
    uint8_t const filter_power = TOUCH_CFG_FILTER_DEPTH;
...
static ctsu_err_t filter(touch_ctrl_blk_t * const p_ctrl)
{
    ...
    for(itr = 0; (itr < num_sensors) ; itr++)
    {
        ...
        ctsu_err = R_CTSU_Read(p_ctrl->ctsu_hw_id, &read_arg);
...
static uint16_t sensors_touched(touch_ctrl_blk_t * const p_ctrl)
{
    ...
    for(itr = 0; (itr < num_sensors); itr++)
    {
        ...
        if (true == action.bit.detection)
        {
            ...
            /* Save parameters that can be changed by the algorithm */
            p_ctrl->p_sensor[itr].dt_count = detect_arg.dt_count;
            p_ctrl->p_sensor[itr].dr_count = detect_arg.dr_count;
            ...
            if(detect_arg.dt_count > 0)
            {
                ...
                instant_touched_sensor_count += 1;
            }
            if(true==touched)
            {
                /* Set bit-mask */
                num_touched_sensors += 1;
...
static ctsu_err_t sensitivity_optimization(touch_ctrl_blk_t * const p_ctrl)
{
    ...
    for(itr = 0; (itr < num_sensors); itr++)
    {
                ...
                )
        {
            ...
            {
                .ctsu_hw_id = p_ctrl->ctsu_hw_id,
                .sensor_offset = itr,
                .input = p_ctrl->p_sensor[itr].input,
                .baseline = p_ctrl->p_sensor[itr].baseline,
                .interval = p_ctrl->p_sensor[itr].p_touch_sensor->recalib_delay,
...
static uint16_t get_sensor_offset(touch_ctrl_blk_t * const p_ctrl, uint8_t rx_ch, uint8_t tx_ch)
{
    ...
    sensor_offset = (CTSU_MODE_MUTUAL_CAPACITANCE == p_ctrl->mode) ?
...
static touch_err_t read_sensor_parameter(touch_ctrl_blk_t * const p_ctrl, touch_read_t const * const p_arg)
{
    ...
    uint16_t const count = (FIT_NO_PTR == p_arg->sensor) ?
...
static void touch_common_callback(void* p_cb_arg)
{
    ...
    touch_ctrl_blk_t * p_ctrl;
    ctsu_callback_arg_t * p_arg = p_cb_arg;

    ctsu_event_t event = (ctsu_event_t)p_arg->event_mask;

    for(itr = 0; itr < TOUCH_CFG_MAX_CONTROL_BLOCK_COUNT; itr++)
    {
        p_ctrl = &pvt_touch_ctrl_blk[itr];
        ...
        if(p_ctrl->ctsu_hw_id==p_arg->handle_num)
        {
