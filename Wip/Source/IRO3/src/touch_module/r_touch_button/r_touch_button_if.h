...
#ifndef R_TOUCH_BUTTON_IF_H_
#define R_TOUCH_BUTTON_IF_H_

#include "r_touch_rx_if.h"

/** Major Revision number of the driver */
#define TOUCH_BUTTON_VERSION_MAJOR  (2)

/** Minor Revision number of the driver */
#define TOUCH_BUTTON_VERSION_MINOR  (00)

#if !defined(TOUCH_VERSION_MAJOR)||!defined (TOUCH_VERSION_MINOR)
#error "TOUCH Middle-ware version not defined."
#endif

#if (TOUCH_VERSION_MAJOR < 2)
...
typedef struct st_touch_button
{
    ...
    touch_cfg_t * const p_touch_cfg;    ///< Pointer to lower level Touch configuration with which this button operates.
    union
    {
        struct{
    ...
    fit_callback_t p_callback;          ///< Function invoked when an button event occurs
 ...
 */
touch_button_err_t R_TOUCH_ButtonOpen(uint32_t * const handle, touch_button_cfg_t const * const btn);
 ...
 */
touch_button_err_t R_TOUCH_ButtonClose(uint32_t handle);
 ...
 */
touch_button_err_t R_TOUCH_ButtonControl(uint32_t handle, touch_button_control_arg_t* p_arg);
